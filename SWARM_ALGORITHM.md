...
The thirteenth implementation step calibrates runtime nudge strength. Runtime nudges now use one uniform base cap of `0.100` across all four policy coordinates, then scale by profile: very low `0.50x`, low `0.75x`, current `1.00x`, strong `1.25x`, and very strong `1.50x`. Policy batches now run `4 policy presets x 5 nudge profiles = 20 runs` on a fixed scenario. Telemetry records base psi, runtime nudge, effective psi, delta psi, nudge profile, average absolute delta, and max absolute delta so we can decide whether adaptation is too weak, useful, or overpowering before Bayesian/GP optimizer work.

The four-by-five nudge sweep is a calibration phase, not the permanent experiment shape. The intended sequence is: run the 20-run nudge calibration batch, inspect whether the effective psi deltas are discernible without destabilizing behavior, freeze one default nudge profile, and then switch to Pareto-only batches that keep that nudge fixed while varying the four main Pareto policy coordinates. The telemetry panel provides a lightweight batch summary for immediate sanity checks, while serious fitting, plotting, and GP/Bayesian optimization can happen outside the sim on exported JSON/CSV data.

Policy batches use fixed-time windows for fair comparisons. The current first-test default is 120 seconds, with the UI allowing 30-900 seconds. Use roughly 45 seconds for smoke tests and 180+ seconds only if AOI contact/dwell metrics show that 120 seconds is too short.

Auto-duration is intentionally deferred. Although a distance/speed/AOI-dwell formula would be fairer across AOIs at different ranges, the next dataset should keep a fixed 120-second budget and compare primarily within each AOI scenario. Cross-AOI distance effects remain recorded through AOI distance, first-contact time, dwell time, path length, and validity flags, so later analysis can model the bias instead of hiding it.

Telemetry exports now separate two needs. JSON is the canonical full-fidelity record with nested samples, control snapshots, and scoring metadata. CSV is the flattened run table intended for notebooks, statistics, sensitivity checks, and the first optimizer/surrogate ingestion path. Runs are also tagged with a dataset workspace name so calibration sweeps, Pareto sweeps, and later adversarial non-combat datasets can be separated without deleting older local records.

Sparse trajectory logging is synchronized with telemetry samples rather than animation frames. Each sample stores per-drone global position, velocity, smoothed target, launch-relative offset, role, assignment, nearest AOI distance, path length, and local scan/point counts. This gives enough temporal structure to explain why a run scored well or poorly without turning every frame into persistent data.

Current run validity is intentionally transparent. Runs remain stored, but are flagged when they are too short, cancelled during a batch, fail to reach AOIs, lack AOI dwell, lack raw AOI coverage, or run with unstable frame time. Invalid runs are filtered/scored down instead of silently deleted.

Future semantic mapping should add rational object-scene relationships before large dataset generation. Object placement should respect terrain and real-world plausibility, semantic labels should be attached to scan hits, and point-cloud/reconstruction exports should preserve those labels for later natural-language-aware 3DGS and world-model work.

Role behavior is currently not a hard exclusive division of labor. All drones perform baseline mapping rays so navigation and global point-cloud evidence remain robust; roles alter task assignment, focused AOI sensing, role count targets, movement speed, network compliance, and derived control gains. The next behavior-kernel refinement should make role differences more explicit: scouts should lead frontier discovery, mappers should own distributed AOI/coverage lanes, relays should prioritize communication topology, and verifiers should revisit/confirm high-value AOI evidence. These role-specific controls should still be driven by the same Pareto/effective-psi and derived-parameter chain.
