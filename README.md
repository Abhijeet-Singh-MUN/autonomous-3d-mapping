...
- Telemetry batch summaries give quick in-sim sanity checks for best run, average fixed score/loss, policy-aligned score/loss, valid/cancelled counts, role entropy, and average Pareto vectors before deeper external analysis.
...
- Saved runs are scored with a fixed equal-weight Pareto evaluator for fair research comparison. A separate psi-weighted score is kept as a policy self-consistency diagnostic, and role entropy is recorded as an interpretable behavior-organization metric. Scoring also stores raw measures, temporal measures, normalization targets, and confidence metadata.
